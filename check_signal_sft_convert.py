...
import os
from sh import gunzip #note: sh and nibabel is not part of the standard install
    ...
    for x in [1, 2]:
        if i==1: #need to check contents
            ...
            if os.path.exists(filename):
                ...
                gunzip(filename)
            #Matlab code: input_nii = cbiReadNifti(strcat(inputnifti,'.nii'));
            input_nii = nib.load(filename) #make sure nib.load does same things as cbiReadNifti
