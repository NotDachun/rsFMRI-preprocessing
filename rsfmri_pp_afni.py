...
def parse_args(args):
	...
	parser.add_argument("-v", "--volumes", action="store", default=0, type=int, metavar="V",
						help="truncate the epi data to the inputted number of volumes, useful if subjects have data with different numbers of volumes (default = no truncation)")
	...
	assert (args.dim_voxel >= 0), "Cannot have a negative voxel dimension"
	assert (not np.all(np.array(result.bandpass) > 0)), "Cannot have a negative frequency limit for bandpassing"
	assert (result.volumes > 0), "Number of volumes must be greater than 0"
	assert (result.cores > 0), "Number of cores used must be greater than 0"
	assert (result.bandpass_ts > 0), "Time step must be greater than 0"
...
def run_SSwarper(out_dir, subj_id, anat, template):
	...
	sswarper_outdir = out_dir + "SSwarper_Output/"
...
def truncate_epi(epi, out_dir, volumes):
	...
	if (subj_vol > volumes):
		record("Truncating to first {} volumnes".format(volumes))
		epi_to990 = out_dir + "to990"
		...
		record(subprocess.check_output([
			"3dTcat", "-prefix", epi_to990,
			"{}'[0..989]'".format(epi)], stderr=subprocess.STDOUT))
		epi_to990 += "+orig."
		return epi_to990
	return epi
...
def generate_afni_proc(args, template):
	...
	sswarper_outdir = args.out_dir + "SSwarper_Output/"
	...
	"-anat_has_skull", "no",
	"dsets", args.epi,
	"-tcat_remove_first_trs", args.trs_remove,
	"-align_opts_aea", "-giant_move", "-cost", "lpc+zz",
	"-volreg_align_to", "MIN_OUTLIER",
	"-volreg_align_e2a",
	"-volreg_tlrc_warp",
	"-tlrc_base", template,
	"-volreg_warp_dxyz", args.dim_voxel]
	...
	if args.nl_reg:
		afni_proc.extend([
			...
			sswarper_outdir + "anatQQ.{}.nii".format(subj_id),
			sswarper_outdir + "anatQQ.{}.aff12.1D".format(subj_id),
			sswarper_outdir + "anatQQ.{}_WARP.nii".format(subj_id)])
	...
	afni_proc.extend([
		...
		"-regress_bandpass", args.bandpass[0], args.bandpass[1],
...
def set_epi_tr(epi, tr):
	...
	curr_tr = subprocess.check_output([
		"3dinfo", "-tr", epi])
	record("The current TR is {}", curr_tr)

	if (not np.isclose(curr_tr, tr)):
		record("Modifying TR from {} to be {}", curr_tr, tr)
		# Equivalent to shell command: 3drefit -TR $epi
		record(subprocess.check_output([
			"3drefit", "-TR", tr, epi]))

	check_tr = subprocess.check_output([
		"3dinfo", "-tr", epi])

	assert (np.isclose(check_tr, tr)), "TR was not modified correctly"
...
def clean(subj_id):
	...
	if (os.path.isdir("{}.results".format(subj_id))):
		shutil.rmtree("{}.results".format(subj_id))
